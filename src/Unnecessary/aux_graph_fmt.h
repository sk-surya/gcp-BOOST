#ifndef _AUX_GRAPH_FMT_H
#define _AUX_GRAPH_FMT_H
...
std::unordered_map<std::string, vertices_size_type> get_LG_idx_map (const MyGraphAdjListD& G_acyclic)
{
    ...
    for (boost::tie(e, e_end) = boost::edges(G_acyclic); e != e_end; e++)
    {
        ...
        strcpy(key_string, "v");
        strcat(key_string, format_int(src).c_str());
        strcat(key_string, "v");
        strcat(key_string, format_int(trg).c_str());
...
{   
    ...
    for (boost::tie(e, e_end) = boost::edges(G_acyclic); e != e_end; e++)
    {
        ...
        strcat(source_key_string, format_int(src).c_str());
        strcat(source_key_string, "v");
        strcat(source_key_string, format_int(trg).c_str());
        sks.assign(source_key_string);
        ...
        for (auto& current_vertex : edge_vec)
        {
            ...
            for (boost::tie(oe, oe_end) = boost::out_edges(current_vertex, G_acyclic); oe != oe_end; oe++)
            {
                ...
                strcpy(keystring, "v");
                strcat(keystring, format_int(source(*oe, G_acyclic)).c_str());
                strcat(keystring, "v");
                strcat(keystring, format_int(target(*oe, G_acyclic)).c_str());
                ks.assign(keystring);
            ...
            for (boost::tie(ie, ie_end) = boost::in_edges(current_vertex, G_acyclic); ie != ie_end; ie++)
            {
                ...
                strcpy(keystring, "v");
                strcat(keystring, format_int(source(*ie, G_acyclic)).c_str());
                strcat(keystring, "v");
                strcat(keystring, format_int(target(*ie, G_acyclic)).c_str());
                ks.assign(keystring);
                if (strcmp(keystring, source_key_string) != 0) // when equal, this entry is not needed (also it'll be duplicated)
                    vertex_strings.push_back(ks);
            }
