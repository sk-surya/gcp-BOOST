...
using boost::spirit::karma::ulong_;
...
std::unordered_map<std::string, vertices_size_type> get_LG_idx_map (const MyGraphAdjListD& G_acyclic)
{
    ...
    for (boost::tie(e, e_end) = boost::edges(G_acyclic); e != e_end; e++)
    {
        ...
        // auto src_str = std::to_string(src);
        src_str.clear();
        trg_str.clear();
        generate(
             std::back_inserter(src_str), // the output
             ulong_,                    // the generator
             src                     // the input
             );
        // auto trg_str = std::to_string(trg);
        generate(
             std::back_inserter(trg_str), // the output
             ulong_,                    // the generator
             trg                     // the input
             );
...
{   
    ...
    std::string src_str, trg_str;
    std::string v_src_str, v_trg_str;
    for (boost::tie(e, e_end) = boost::edges(G_acyclic); e != e_end; e++)
    {
        edge_counter++;
        //std::cout << source(*e, G_acyclic) << ", " << target(*e, G_acyclic) << std::endl;
        ...
        */
        ...
        for (auto& current_vertex : edge_vec)
        {
            ...
            for (boost::tie(oe, oe_end) = boost::out_edges(current_vertex, G_acyclic); oe != oe_end; oe++)
            {
                ...
                v_src_str.clear();
                v_trg_str.clear();
                generate(std::back_inserter(v_src_str), ulong_, source(*oe, G_acyclic)); //alternative to to_string
                generate(std::back_inserter(v_trg_str), ulong_, target(*oe, G_acyclic));
                //auto keystring =   "v" + std::to_string(source(*oe, G_acyclic)) 
                //                + "v" + std::to_string(target(*oe, G_acyclic));
                auto keystring =   "v" + v_src_str + "v" + v_trg_str;
            ...
            for (boost::tie(ie, ie_end) = boost::in_edges(current_vertex, G_acyclic); ie != ie_end; ie++)
            {
                ...
                v_src_str.clear();
                v_trg_str.clear();
                generate(std::back_inserter(v_src_str), ulong_, source(*ie, G_acyclic)); //alternative to to_string
                generate(std::back_inserter(v_trg_str), ulong_, target(*ie, G_acyclic));
