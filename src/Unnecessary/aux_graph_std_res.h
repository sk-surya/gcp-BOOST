#pragma once
#ifndef _AUX_GRAPH_STD_RES_H
#define _AUX_GRAPH_STD_RES_H
...
std::unordered_map<std::string, vertices_size_type> get_LG_idx_map (const MyGraphAdjListD& G_acyclic)
{
    ...
    auto numD = numDigits(num_vertices(G_acyclic));
    src_str.reserve(numD);
    trg_str.reserve(numD);
...
{   
    ...
    std::string src_str, trg_str, keystring, source_key_string;
    auto numD = numDigits(num_vertices(G_acyclic));
    src_str.reserve(numD);
    trg_str.reserve(numD);   
    keystring.reserve(2*numD + 2);
    source_key_string.reserve(2*numD + 2);       
    for (boost::tie(e, e_end) = boost::edges(G_acyclic); e != e_end; e++)
    {
        edge_counter++;
        //std::cout << source(*e, G_acyclic) << ", " << target(*e, G_acyclic) << std::endl;
        ...
        source_key_string = "v" + src_str + "v" + trg_str;
        ...
        */
        ...
        for (auto& current_vertex : edge_vec)
        {
            ...
            for (boost::tie(oe, oe_end) = boost::out_edges(current_vertex, G_acyclic); oe != oe_end; oe++)
            {
                ...
                keystring =   "v" + std::to_string(source(*oe, G_acyclic)) 
            ...
            for (boost::tie(ie, ie_end) = boost::in_edges(current_vertex, G_acyclic); ie != ie_end; ie++)
            {
                ...
                keystring =   "v" + std::to_string(source(*ie, G_acyclic)) 
