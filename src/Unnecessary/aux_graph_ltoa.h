#ifndef _AUX_GRAPH_LTOA_H
#define _AUX_GRAPH_LTOA_H
    ...
    for (boost::tie(e, e_end) = boost::edges(G_acyclic); e != e_end; e++)
    {
        ...
        for (auto& current_vertex : edge_vec)
        {
            ...
            for (boost::tie(oe, oe_end) = boost::out_edges(current_vertex, G_acyclic); oe != oe_end; oe++)
            {
                ...
                strcpy(keystring, "v");
                strcat(keystring, src_str);
                strcat(keystring, "v");
                strcat(keystring, trg_str);
                ks = keystring;
            ...
            for (boost::tie(ie, ie_end) = boost::in_edges(current_vertex, G_acyclic); ie != ie_end; ie++)
            {
                ...
                char v_src_str[numD];
                char v_trg_str[numD];
                ltoa(source(*ie, G_acyclic), v_src_str, 10);
                ltoa(target(*ie, G_acyclic), v_trg_str, 10);
        ...
        // std::cout << vertex_strings.size() << std::endl;
    }
